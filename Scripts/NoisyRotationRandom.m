function [MRerror,MOerror,MTerror]=NoisyRotationRandom(varargin)
  % [MeanRerror,MeanOerror,MeanTerror]=NoisyRotationRandom;
  %   Generates Fig. 3, rigth-column
  %
  % Optional parameters:
  ...
  maxRotError=0.1; % maximum rotation error (0.05)
  ...
  errorbound=linspace(0, maxRotError, samples_error);
  ...
  for s=1:samples_error    % level of error
    ...
    for j=1:m
      ...
      for i=1:n
        B{i}=RandomTransf();
        A{i}=X*B{i}*X^-1*RotPerturbation(errorbound(s));
      end
  ...
  if errorBars
    ...
    for k=1:nMethods
      semilogy(errorbound,squeeze(MRerror(:,k)),color{k},'LineWidth',lw);
      hold on;
    end
  end
  h=gca;
  ...
  xlabel('Level of noise ($\sigma_{\rm rot}$)','Interpreter','latex','FontSize',fs);
  grid on;
  ...
  if errorBars
    ...
    for k=1:nMethods
      semilogy(errorbound,squeeze(MOerror(:,k)),color{k},'LineWidth',lw);
      hold on;
    end
  end
  h=gca;
  ...
  if errorBars
    ...
    for k=1:nMethods
      semilogy(errorbound,squeeze(MTerror(:,k)),color{k},'LineWidth',lw);
      hold on;
    end
  end
  h=gca;
